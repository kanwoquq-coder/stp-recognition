...
class OpenAICompatibleChatGateway:
    """统一管理 OpenAI 兼容模型的超时、重试、JSON模式和视觉降级。"""

    def __init__(self, api_key: str, base_url: str = None,
                 timeout: float = 180.0, max_retries: int = 1,
                 response_format_mode: str = "auto",
                 thinking_mode: str = "auto"):
        response_format_mode = response_format_mode.lower().strip()
        thinking_mode = thinking_mode.lower().strip()
        if response_format_mode not in {"auto", "json", "none"}:
            raise ValueError("response_format_mode 必须是 auto、json 或 none")
        if thinking_mode not in {"auto", "on", "off"}:
            raise ValueError("thinking_mode 必须是 auto、on 或 off")
    ...
    def create_json_completion(self, model: str, messages: list,
        ...
        try:
            ...
            # 部分 vLLM/SGLang 服务未实现 response_format。仅在确认是该参数
            # 不兼容时去掉参数重试，避免把网络故障误判为兼容性问题。
            if (
                self.response_format_mode == "auto"
    ...
    def _is_visual_request_failure(self, error: Exception) -> bool:
        ...
        gateway_error_in_text = bool(
            re.search(r"\b(?:413|415|422|500|501|502|503|504)\b", message)
        )
        return (
    ...
    def _disable_vision_or_service(self, model: str, visual_error: Exception):
        ...
        try:
            ...
            self.text_disabled_reason = (
                f"LLM服务器视觉请求失败（{status_text}），"
                f"纯文本探测也失败: {probe_error}"
            )
        ...
        self.vision_disabled_reason = (
            f"LLM服务器纯文本接口正常，但图片请求失败（{status_text}）；"
...
def parse_llm_json(raw_content):
    """解析纯JSON、Markdown代码块或带思考文本的Qwen响应。"""
    result = raw_content
    for _ in range(5):
...
def _extract_occ_bbox(filepath: str) -> Optional[List[float]]:
    """Use the CAD kernel when STEP text references cannot produce a bbox."""
    try:
        from OCC.Core.STEPControl import STEPControl_Reader
        ...
        if reader.ReadFile(str(filepath)) != 1:
            return None
        ...
        dims = sorted(round(float(value), 4) for value in (
            xmax - xmin, ymax - ymin, zmax - zmin
        ))
        if all(math.isfinite(value) and value > 0 for value in dims):
            return dims
    except Exception:
        return None
    return None
...
def parse_stp_deep(filepath: str) -> dict:
    ...
    # ISO 10303-21 exchange files are text and carry this signature near the
    # beginning. Binary/encrypted/renamed uploads must not silently become a
    # zero feature vector, which otherwise looks like a valid perfect match.
    if b'ISO-10303-21' not in raw_content[:4096].upper():
        raise ValueError(
            "不是有效的STEP Part 21文件：文件头缺少 ISO-10303-21，"
            "可能是加密/压缩文件、其他CAD格式被改名或上传内容损坏"
        )
    ...
    if not entities:
        raise ValueError("STEP文件没有可解析的DATA实体")
    ...
    if points:
        ...
        occ_dims = _extract_occ_bbox(filepath)
        info['bbox_dims'] = occ_dims or [0, 0, 0]
        info['bbox_diagonal'] = (
            round(math.sqrt(sum(d**2 for d in occ_dims)), 4) if occ_dims else 0
        )
        if occ_dims:
            info['bbox_source'] = 'occ_shape'
        info['point_distribution_ratios'] = [1.0, 1.0, 1.0]
...
def _safe_dims(info: dict) -> list[float]:
    dims = info.get('bbox_dims') or [0, 0, 0]
    dims = [_safe_number(value) for value in list(dims)[:3]]
    while len(dims) < 3:
        dims.append(0.0)
    return dims


def _format_dims(dims: list[float]) -> str:
    if not all(value > 0 for value in dims):
        return "无法从STEP实体提取（不要按0尺寸解释）"
    return f"{dims[0]:.2f} x {dims[1]:.2f} x {dims[2]:.2f} mm"


def _dominant_shape_from_dims(dims: list[float]) -> str:
    small, mid, large = sorted(dims)
    if large <= 0:
        return "尺寸未知"
    if small / large < 0.12:
        return "薄板/片状"
    if mid / large < 0.35:
        return "长条/梁状"
    if max(dims) / max(min(dims), 1e-6) < 1.35:
        return "近块状"
    return "块状/支架类"


def _format_radii(values: list, limit: int = 8) -> str:
    radii = [_safe_number(value) for value in values or []]
    radii = [value for value in radii if value > 0]
    if not radii:
        return "无"
    text = ", ".join(f"R{value:.2f}" for value in radii[:limit])
    if len(radii) > limit:
        text += f" 等{len(radii)}种"
    return text


def _format_count_map(values: dict, limit: int = 8) -> str:
    if not isinstance(values, dict) or not values:
        return "无"
    items = [(key, count) for key, count in values.items() if count]
    if not items:
        return "无"
    return ", ".join(f"{key}:{count}" for key, count in items[:limit])


def _format_mfg_features(mfg: dict) -> str:
    ...
    for key, label in names:
        count = int(_safe_number(mfg.get(key, 0)))
        if count > 0:
            parts.append(f"{label}{count}个")
    return "、".join(parts) if parts else "未识别到明确制造特征"


def generate_description(info: dict) -> str:
    """基于STEP解析结果生成确定性零件描述，避免让LLM凭图片臆测结构。"""
    lines = []
    dims = _safe_dims(info)
    ar = info.get('aspect_ratios') or [0, 0]
    ar = [_safe_number(value) for value in list(ar)[:2]]
    while len(ar) < 2:
        ar.append(0.0)
    mfg = info.get('mfg_features', {})

    lines.append(f"零件名称: {info.get('filename', '')}")
    lines.append(f"整体形态: {_dominant_shape_from_dims(dims)}")
    lines.append(f"包围盒尺寸(小/中/大): {_format_dims(dims)}")
    if all(value > 0 for value in dims):
        lines.append(f"包围盒对角线: {_safe_number(info.get('bbox_diagonal')):.2f} mm")
        lines.append(f"尺寸比例: 中/小={ar[0]:.2f}, 大/中={ar[1]:.2f}")
    else:
        lines.append("包围盒对角线: 未知")
        lines.append("尺寸比例: 未知")
    lines.append(
        f"拓扑规模: 面{int(_safe_number(info.get('num_faces')))}个, "
        f"边{int(_safe_number(info.get('num_edges')))}条, "
        f"顶点{int(_safe_number(info.get('num_vertices')))}个, "
        f"边环{int(_safe_number(info.get('num_edge_loops')))}个"
    )
    lines.append(f"面类型统计: {_format_count_map(info.get('face_types', {}))}")
    lines.append(f"边类型统计: {_format_count_map(info.get('edge_types', {}))}")
    lines.append(
        f"圆/孔半径特征: {_format_radii(info.get('unique_radii', []))}; "
        f"圆柱面半径: {_format_radii(info.get('cylinder_radii', []))}"
    )
    lines.append(
        f"制造特征: {_format_mfg_features(mfg)}; "
        f"特征密度={_safe_number(mfg.get('feature_density')):.3f}, "
        f"复杂度={_safe_number(mfg.get('complexity_score')):.3f}"
    )
    lines.append(
        "几何估计: "
        f"表面积约{_safe_number(info.get('total_surface_area_estimate')):.2f}, "
        f"体积约{_safe_number(info.get('volume_estimate')):.2f}, "
        f"紧密度{_safe_number(info.get('compactness')):.3f}"
    )
...
def generate_retrieval_description(info: dict) -> str:
    ...
    cylinder_radii = info.get('cylinder_radii', [])
    mfg = info.get('mfg_features', {})
    return '\n'.join([
        f"整体形态: {_dominant_shape_from_dims(dims)}",
        f"包围盒尺寸(小/中/大): {_format_dims(dims)}",
        (f"包围盒对角线: {_safe_number(info.get('bbox_diagonal')):.2f} mm"
         if all(value > 0 for value in dims) else "包围盒对角线: 未知"),
        (f"包围盒比例: {ar[0]:.4f}, {ar[1]:.4f}"
         if all(value > 0 for value in dims) else "包围盒比例: 未知"),
        ...
        "圆特征半径: " + _format_radii(radii),
        "圆柱面半径: " + _format_radii(cylinder_radii),
        "制造结构代理: " + _format_mfg_features(mfg),
    ])
...
class VisualVectorIndex:
    ...
    def remove_vector(self, filepath: str) -> bool:
        """删除一个文件的视觉向量，并压缩重建 IndexFlatIP 索引。"""
        if not HAS_FAISS or self.index is None:
            return False
        target = os.path.normcase(os.path.abspath(filepath))
        matched = next(
            (
                path for path in self._id_to_idx
        ...
        all_vectors = (
            ...
            if self.index.ntotal else np.empty((0, self.dim), dtype=np.float32)
        )
        remaining = [
            (path, all_vectors[idx])
            for idx, path in sorted(self._idx_to_id.items())
            if path != matched and 0 <= idx < len(all_vectors)
        ]
        self.reset()
        for path, vector in remaining:
            self.add_vector(path, vector)
        if not self.save():
            raise RuntimeError("视觉索引保存失败")
        return True
...
class GeoVectorIndex:
    ...
    def remove_vector(self, filepath: str) -> bool:
        """删除一个文件的几何向量，并压缩重建 IndexFlatIP 索引。"""
        if not HAS_FAISS or self.index is None:
            return False
        target = os.path.normcase(os.path.abspath(filepath))
        matched = next(
...
class EmbeddingIndex:
    ...
    def delete_file(self, filepath: str) -> dict[str, bool]:
        """从文本、几何、视觉三路索引同步删除指定STP文件。"""
        if self.use_faiss:
            raise RuntimeError("后端FAISS文本索引模式暂不支持按文件删除")
        ...
        for item_id, metadata in zip(
            ...
            stored_path = (metadata or {}).get("path")
            if stored_path and os.path.normcase(os.path.abspath(stored_path)) == target:
                delete_ids.append(item_id)
        if delete_ids:
            self.collection.delete(ids=delete_ids)

        geo_deleted = self.geo_index.remove_vector(filepath) if self.geo_index else False
        visual_deleted = (
        ...
        return {
            "text": bool(delete_ids),
            "geometric": geo_deleted,
            "visual": visual_deleted,
        }
...
{generate_retrieval_description(query_info)}"""
        ...
        if self.visual_index:
            ...
            # 如果视图不足，尝试渲染
            valid_views = [v for v in (view_paths or []) if v and Path(v).exists()]
            if auto_render and len(valid_views) < 3 and HAS_RENDER:
    ...
    def _render_views(self, stp_path: str, view_dir: str = None) -> Optional[List[str]]:
        """Use the same canonical renderer as uploads and online rendering."""
        return STPRenderer(cache_dir=view_dir).render_to_cache(stp_path)
...
class STPRenderer:
    ...
    def _render_mesh_views(self, mesh, output_dir: Path, base_name: str) -> List[str]:
        """Canonical geometry, explicit right-handed cameras and shared scale."""
        from view_alignment import render_standard_views
        paths, metadata = render_standard_views(mesh, output_dir, base_name)
    ...
    def render_to_cache(self, stp_path: str, force: bool = False) -> Optional[List[str]]:
        ...
        from view_alignment import render_cache_valid, source_signature
        if not force and render_cache_valid(stp_path, cache_subdir):
            print(f"  [Render] 使用缓存视图: {base_name}")
            return [str(p) for p in cached_views]

        print(f"  [Render] 正在渲染: {base_name}")
        # A failed refresh must not leave a valid marker for partial images.
        (cache_subdir / "render_manifest.json").unlink(missing_ok=True)
        ...
        try:
            ...
            metadata = {**self._last_render_metadata, "source": source_signature(stp_path)}
            (cache_subdir / "render_manifest.json").write_text(
...
返回JSON格式（所有字段都必须填写）：
{
  ...
  "summary": "一句话总结"
    ...
    def __init__(self, api_key: str, base_url: str = None,
        ...
        # vLLM 的多模态服务通常会通过 --limit-mm-per-prompt.image
        # 限制单次请求的图片数。Qwen3.5 的常见部署值为 1，因此默认只发送
        # 一张代表视图；确认服务端支持多图后可设置 STP_LLM_MAX_IMAGES=3/6。
        self.max_images_per_request = max(
            1, int(os.getenv("STP_LLM_MAX_IMAGES", "1"))
        )
    ...
    def extract_features(self, view_paths: List[str], stp_text_features: str = None) -> Dict:
        ...
        if self.vision_unavailable or not self.chat_gateway.vision_available:
            return None

        # 检查缓存
        from view_alignment import source_signature
        cache_key = json.dumps({"views": [source_signature(p) for p in valid_views],
                                "text": stp_text_features, "model": self.model}, sort_keys=True)
        if cache_key in self._feature_cache:
            print(f"  [特征提取] 使用缓存特征")
            return self._feature_cache[cache_key]

        request_views = valid_views[:self.max_images_per_request]
        print(
            f"  [特征提取] 正在分析 {len(request_views)} 个代表视图"
            f"（本地共有 {len(valid_views)} 个）..."
        )

        # 准备图片消息
        image_messages = prepare_view_image_messages(request_views, include_names=True)
        ...
        try:
            ...
            detail = f"；服务端信息: {error_body}" if error_body else ""
            print(f"  [特征提取] 错误: {e}{detail}")
            if (
                not self.chat_gateway.vision_available
                or status_code in {400, 413, 415, 422, 500, 501, 502, 503, 504}
            ):
                self.vision_unavailable = True
                print(
                    "  [视觉熔断] 当前Qwen服务不能处理该图片请求，"
                    "本次检索停止继续调用视觉接口并回退到文本/几何精排"
                )
            return None
...
def prepare_view_image_messages(view_paths: List[str], include_names: bool = True) -> List[dict]:
    ...
    Returns:
        消息内容列表
    """
    from view_alignment import named_views
    view_names = {"front": "主视图", "back": "后视图", "top": "俯视图",
    ...
    for i, path in enumerate(view_paths):
        ...
        if include_names:
            identified = named_views([path])
            label = view_names[next(iter(identified))] if identified else f"未标定视图{i + 1}"
            messages.append({
                "type": "text",
                "text": f"\n【{label}】"
            })
...
class LLMReranker:
    def __init__(self, api_key: str, base_url: str = None,
        ...
        Args:
            ...
            timeout: 单次模型请求超时秒数
            max_retries: OpenAI客户端自动重试次数
            response_format_mode: JSON响应模式，auto/json/none
            thinking_mode: Qwen思考模式，auto/on/off
        """
        ...
        print(
            f"  [LLM配置] model={model}, timeout={timeout}s, retries={max_retries}, "
            f"response_format={response_format_mode}, thinking={thinking_mode}"
        )
    ...
    @staticmethod
    def _original_rankings(candidates: list[dict], top_k: int,
                           reason: str = "使用召回原始排序") -> list[dict]:
        """LLM不可用时保留已经得到的三路融合/几何排序。"""
        return [
            LLMReranker._ranking_from_candidate(i, c, reason=reason)
            for i, c in enumerate(candidates[:top_k])
        ]

    @staticmethod
    def _ranking_from_candidate(index: int, candidate: dict,
                                ranking: dict | None = None,
                                reason: str = "使用召回原始排序") -> dict:
        fallback_score = candidate.get(
            'hybrid_similarity',
            candidate.get('fusion_score', candidate.get('similarity', 0.5)),
        )
        item = {
            ...
            'reason': reason,
            'path': candidate.get('path', candidate.get('filepath', '')),
            'filepath': candidate.get('filepath', candidate.get('path', '')),
            'embedding_similarity': candidate.get(
                'text_similarity',
                candidate.get('embedding_similarity', candidate.get('similarity', 0)),
            ),
            'text_similarity': candidate.get(
                'text_similarity',
                candidate.get('embedding_similarity', candidate.get('similarity', 0)),
            ),
            'geo_similarity': candidate.get('geo_similarity', 0),
            'geometric_similarity': candidate.get('geometric_similarity', {}),
            'visual_similarity': candidate.get('visual_similarity', 0),
            'fusion_score': candidate.get('fusion_score', 0),
            'final_score': candidate.get(
                'final_score',
                candidate.get('hybrid_similarity', candidate.get('fusion_score', 0)),
            ),
            'hybrid_similarity': candidate.get(
                'hybrid_similarity',
                candidate.get('fusion_score', candidate.get('similarity', 0)),
            ),
            'similarity': candidate.get('similarity', fallback_score),
        ...
        if isinstance(ranking, dict):
            item.update(ranking)
            # 身份和索引/几何/视觉分数必须以本地候选数据为准，不能信任模型生成。
            for key in (
                'candidate_number', 'filename', 'path', 'filepath',
                'embedding_similarity', 'text_similarity', 'geo_similarity',
                'geometric_similarity', 'visual_similarity', 'fusion_score',
                'final_score', 'hybrid_similarity', 'similarity',
                'recall_sources', 'recall_count', 'description',
            ):
                item[key] = {
                    'candidate_number': index + 1,
                    'filename': candidate.get('filename', ''),
    ...
    @classmethod
    def _complete_rankings(cls, rankings: list, candidates: list[dict],
                           top_k: int) -> list[dict]:
        """校验LLM排名，并用召回顺序补齐模型漏掉的候选。

        大模型并不保证严格返回提示词要求的条数，尤其首次请求时可能只
        返回示例中常见的5条。接口层还需要过滤查询零件本身，因此这里
        必须尽量返回 ``top_k`` 条有效且不重复的候选，才能保证最终数量。
        """
        target_count = min(max(int(top_k), 0), len(candidates))
        if target_count == 0:
            return []

        filename_to_index = {
            str(candidate.get('filename', '')): index
        ...
        def append_candidate(index: int, ranking: dict | None = None) -> None:
            if index in used_indexes or not (0 <= index < len(candidates)):
                return
            candidate = candidates[index]
            item = cls._ranking_from_candidate(
                index,
                candidate,
                ranking,
                reason='LLM未返回该候选，按召回综合分数补齐',
            )
        ...
        if isinstance(rankings, list):
            for ranking in rankings:
                ...
                index = None
                try:
                    candidate_number = int(ranking.get('candidate_number'))
                    if 1 <= candidate_number <= len(candidates):
                        index = candidate_number - 1
                except (TypeError, ValueError):
                    pass
                if index is None:
                    index = filename_to_index.get(str(ranking.get('filename', '')))
                if index is not None:
                    append_candidate(index, ranking)
                if len(completed) >= target_count:
                    break

        llm_result_count = len(completed)
        if len(completed) < target_count:
            for index in range(len(candidates)):
                ...
                if len(completed) >= target_count:
                    break
            print(
                f"  [LLM精排] 模型仅返回 {llm_result_count} 条有效排名，"
                f"已按召回综合分数补齐到 {len(completed)} 条"
            )
...
{reference_constraint}
【事实约束】
- 只能依据下方 STP 解析字段、几何相似度分项和明确给出的用户条件判断。
- 不得编造未出现的结构名称、用途、孔位、图片内容或尺寸。
- reason 必须引用具体数字，例如包围盒尺寸、面数、孔/圆半径、几何相似度分项。
...
必须返回恰好 {min(top_k, len(candidates))} 条不同候选，不得省略候选；返回JSON，按相似度从高到低:
{{
  "rankings": [
    {{
      "candidate_number": <编号>,
      ...
      "reason": "<判断理由>"
        ...
        try:
            ...
            print(f"  [LLM精排] 请求失败: {e}，使用三路融合/几何原始排序")
            return self._original_rankings(candidates, top_k, "LLM服务不可用，使用原始排序")

        # 检查响应是否有效
        if not response.choices or not response.choices[0].message.content:
            print(f"  [警告] LLM返回空响应，使用原始排序")
            return self._original_rankings(candidates, top_k, "LLM响应为空")

        try:
            result = parse_llm_json(response.choices[0].message.content)
        except json.JSONDecodeError as e:
            print(f"  [警告] JSON解析失败: {e}，使用原始排序")
            return self._original_rankings(candidates, top_k, "JSON解析失败")

        rankings = self._complete_rankings(
            result.get('rankings', []), candidates, top_k
        )
        return rankings
...
{reference_constraint}
【事实约束】
- 文字理由优先引用 STP 解析出的尺寸、拓扑、面类型、圆/孔半径和相似度分项。
- 不得编造未出现的零件用途、孔位数量、结构名称或尺寸。
- 如果图片模型能力不足或视觉描述不确定，reason 中应说明依据来自几何解析。
    ...
    {{
      ...
      "reason": "<判断理由>"
        ...
        try:
            ...
            print(f"  [视觉精排] 请求失败: {e}，回退到文本LLM精排")
            return self.rerank(
                query_desc, candidates, top_k,
        ...
        for r in rankings:
            ...
            if 0 <= idx < len(candidates):
                preserved = self._ranking_from_candidate(idx, candidates[idx], r)
                r.clear()
                r.update(preserved)
                # 添加视图信息
...
{query_desc}"""
        ...
        # 服务端一旦明确拒绝首个图片请求，继续分析20个候选只会重复产生
        # 502。直接用已经取得的STP文本/几何特征调用文本精排。
        if (
            self.feature_extractor.vision_unavailable
            or not self.chat_gateway.vision_available
        ):
            print("  [回退] 视觉模型不可用，跳过候选视觉特征提取")
            return self.rerank(
                query_desc, candidates, top_k,
                reference_dimensions=reference_dimensions,
                custom_query_text=custom_query_text,
...
{reference_constraint}
【事实约束】
- 最终 reason 必须以 STP 解析字段为主，引用具体尺寸、拓扑数量、圆/孔半径或相似度分项。
- 不得把视觉识别结果当作确定事实；视觉特征只作辅助。
- 不得编造未出现的零件用途、孔位、结构名称或尺寸。
...
必须返回恰好 {min(top_k, len(candidates))} 条不同候选，不得省略候选。请返回JSON格式的排名结果：
{{
  "rankings": [
    {{
      "candidate_number": <编号>,
      ...
      "reason": "<综合判断理由>"
        ...
        # 每个零件的图片已经在阶段1/2单独转换为结构化视觉特征。
        # 最终精排只发送这些特征文本，避免把查询件和20个候选的约126张
        # 图片塞进一个请求，超过vLLM的图片数量/上下文限制。
        content = [{"type": "text", "text": prompt}]
...
class STPSearchEngine:
    def __init__(self, api_key: str, base_url: str = None,
        ...
        Args:
            ...
            llm_model: LLM模型名称
            llm_api_key: 独立的LLM API密钥；不传时复用api_key
            llm_base_url: 独立的LLM服务地址；不传时复用base_url
            db_path: 向量数据库路径
            use_faiss: 是否使用FAISS索引（后端索引格式）
            view_dir: 视图图片根目录（用于视觉精排）
            render_cache_dir: 渲染缓存目录
            enable_render: 是否启用自动渲染功能
            llm_timeout: 单次LLM请求超时秒数
            llm_max_retries: LLM客户端自动重试次数
            llm_response_format: JSON响应模式，auto/json/none
        ...
        self.reranker = LLMReranker(
            ...
            llm_base_url if llm_base_url is not None else base_url,
            llm_model,
            view_dir=view_dir,
            timeout=llm_timeout,
    ...
    def search(self, query_path: str,
        ...
        # 空索引或空召回时没有可供精排的候选，直接返回，避免无意义地调用 LLM。
        if not coarse:
            print("  [结束] 没有候选件，跳过 LLM 精排")
            return []
        ...
        llm_rerank_candidates = min(20, len(coarse))  # LLM 精排候选最多20个
        ...
        if use_vision and self.renderer:
            ...
            if query_views is None:
                ...
                # 如果没有视图，尝试自动渲染
                if auto_render and available_query_views < 3:
                ...
                elif available_query_views < 3:
                    print(f"  [渲染] 找到 {available_query_views} 个现有视图，auto_render=false，跳过补渲染")
                else:
            ...
            if self.view_dir or self.renderer:
                for c in llm_input:
                    ...
                    # 如果候选零件视图不足，尝试渲染
                    if auto_render and available_cand < 3 and self.renderer:
                        cand_name = Path(c['filename']).stem
...
半径列表: {query_info['unique_radii']}"""
        ...
        if use_vision_rerank and use_3d_vision and self.renderer and auto_render:
            print(f"\n[3D Vision] 正在渲染旋转动画...")
            # 渲染查询零件旋转动画
        ...
        if use_vision_rerank:
            from view_alignment import align_view_files, VIEW_NAMES, named_views
            # Standard views are paired before both visual feature extraction
            # and direct multimodal reranking. Missing views retain old fallback.
            for view_index, views in enumerate(candidate_view_list):
                if len(named_views(query_views)) == 6 and len(named_views(views)) == 6:
                    try:
                        alignment = align_view_files(
                            query_views, views, self.renderer.cache_dir / "alignments"
                        )
                        candidate_view_list[view_index] = [alignment["aligned_views"][n] for n in VIEW_NAMES]
                    except (OSError, ValueError) as exc:
                        print(f"  [视图配对] 候选{view_index + 1}未配对: {exc}")
        ...
        if use_vision_rerank and use_feature_extraction:
            ...
            ranked = self.reranker.rerank_with_feature_extraction(
                ...
                renderer=self.renderer if auto_render else None,
                reference_images=reference_images,
                reference_dimensions=reference_dimensions,
                custom_query_text=custom_query_text
            )
        ...
        for i, r in enumerate(ranked, 1):
            ...
            print(f"     LLM评分: {r['similarity_score']:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base-url", default=None,
                        help="Embedding API base URL，例如 https://dashscope.aliyuncs.com/compatible-mode/v1")
    parser.add_argument("--llm-api-key", default=None,
                        help="独立的LLM精排API密钥；不传时复用--api-key")
    parser.add_argument("--llm-base-url", default=None,
                        help="独立的LLM精排服务地址，例如 http://10.100.0.34:8223/v1")
    parser.add_argument("--llm-model", default="qwen2.5-72b-instruct")
    parser.add_argument("--llm-timeout", type=float, default=180.0,
                        help="单次LLM请求超时秒数（默认180）")
    parser.add_argument("--llm-max-retries", type=int, default=1,
                        help="LLM客户端自动重试次数（默认1）")
    parser.add_argument("--llm-response-format", choices=["auto", "json", "none"],
                        default="auto", help="JSON响应模式兼容策略")
    parser.add_argument("--llm-thinking", choices=["auto", "on", "off"],
                        default="auto", help="Qwen思考模式控制")
    ...
    parser.add_argument("--query-views", nargs='+', default=None,
                        help="查询零件视图图片路径列表；至少提供3张，通常为六视图")
    ...
    parser.add_argument("--visual-recall", type=int, default=150,
                        help="视觉召回数量（默认150）")
    parser.add_argument("--fusion-top", type=int, default=20,
                        help="融合排序后保留数量（默认20）")
    parser.add_argument("--auto-render", action="store_true",
                        help="检索时视图不足则现场补渲染；默认关闭，建议在入库阶段完成渲染")
...
# 安全示例（不要把真实密钥写入代码或命令历史）：
# python stp_similarity.py search --query "C:\path\query.stp" --top 7 \
#   --api-key "<EMBEDDING_API_KEY>" --base-url "<EMBEDDING_BASE_URL>/v1" \
#   --embed-model text-embedding-v3 --db-path "C:\path\chroma_parts" \
#   --llm-api-key "EMPTY" --llm-base-url "http://127.0.0.1:18223/v1" \
#   --llm-model "Qwen3.5-27B" --use-three-way --fusion-top 20 \
#   --view-dir "C:\path\pic"
#.\run_stp_similarity.ps1 -Query "C:\Users\phillip\Desktop\project\teststp\测试件3.stp" -Top 7 -Vision -QueryViewRoot "C:\Users\phillip\Desktop\project\3D" -CandidateViewDir "C:\Users\phillip\Desktop\project\pic"
# Qwen/vLLM 默认每次只发送一个代表视图。服务端确认允许多图后，可在运行前设置：
# PowerShell: $env:STP_LLM_MAX_IMAGES = "3"
# $env:EMBEDDING_API_KEY="your_embedding_api_key"
#$env:EMBEDDING_BASE_URL="https://llm-8qgclixatgifoso3.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
#$env:EMBEDDING_MODEL="qwen-plus"
